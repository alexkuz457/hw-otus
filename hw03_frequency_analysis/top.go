...
func Top10(str string) []string {
	...
	// весь слайс мы тоже оставить не можем из соображений производительности
	// (хотя это было бы проще всего)
	// поэтому мы берем первые 10 отличающихся количеств вхождений слова.
	// это значит что если все слова входят в текст по 2 раза нам придется
	// взять весь слайс, но по-другому никак,
	// однако в большинстве случаев слайс после такой частичной обрезки
	// станет меньше и лишние слова сортировать не придется.
	minLen := 0
	...
	// (думаю впринципе можно было бы обойтись одной этой сортировкой,
	// на текстах типа Вини-пуха разница 0.1 сек.
	// чтобы почувстововать разницу в качестве текста нужно загонять британскую энциклопедию)
	sort.Slice(sortSlice, func(i, j int) bool {
		var res bool
